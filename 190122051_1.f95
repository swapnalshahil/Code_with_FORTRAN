...
PROGRAM main
    ...
    real*4 x, alpha1,alpha2,alpha3,ans
    ...
    alpha2 = 30
    alpha3 = 40.5
    call bino(x,alpha1,ans)
    write(6,*)ans 
    ans = 0
    call bino(x,alpha2,ans)
    write(6,*)ans 
    ans = 0
    call bino(x,alpha3,ans)
    write(6,*)ans 
    
    !CLOSE(outunit)
END PROGRAM main

subroutine bino(x,alpha,ans)
    real*4 x,alpha,ans,term,factn,deno,numer
    integer i ,n 
    
    call factorial(alpha,ans)
    factn = ans
    term = 0.0
    do i=1,int(alpha)+1 
        call factorial(real(i),fact)
        numer = fact 
        call factorial(real(n-i),fact)
        deno = fact
        term = term + factn * x**(i)/(deno * numer)
    end do 
    ans = term
end subroutine bino

subroutine factorial(num,fact)
    integer i
    real*4 fact,num
    fact =1.0
    do i=1,int(num)+1
