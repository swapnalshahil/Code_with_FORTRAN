...
PROGRAM main
    ...
    integer n,i,j ,grid_point
    real*8 dx,xst,xe,dt,xend ,t,x
    complex*16 iota, v(200),psi(200),ans
    ...
    do i=1,200
        ...
        do j=1,grid_point
            x = xst + (j)*dx
            v(i) = v(i) + (exp(-2*(x-xst)) -2*exp(-(x-xe))) *dx 
        end do
    end do 
    ...
    do i=1,200
        ...
        do j=1,grid_point
            x = xst + (j)*dx
            psi(i) = psi(i) + (exp(-2*(x-xe)**2)+ x*exp(-2*(x-xe)**2))**dx
        end do
    end do
    do i=1,200
        t = t + dt
        ans = conjg(v(i))*psi(i)*v(i)
        if(t.ge.2.0 .and. t .le. 2.0 + dt)then
            write(6,*) ans 
        else if(t .eq. 10.0 .and. t .le. 10.0 + dt)then
            write(6,*) ans
        end if 
    end do
    write(6,*) ans
