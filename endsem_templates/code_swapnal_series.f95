...
subroutine sort(a,n)
  ...
  subroutine invert_matrix(arr, n, inv_mat)
  integer i,j,n
  real*8 arr(n,n),t(n,n),inv_mat(n,n)
  t = 0.d0
  inv_mat = 0.d0
  do i=1,n
     if(arr(i,i) .eq. 0.d0)then
        if(i+1 > n) then
           write(6,*) "NO SOLUTION"
        else
           t(i,:) = arr(i,:)
     ...
     do j=1,n
        if(i .ne. j) then
           ...
           inv_mat(j,:) = inv_mat(j,:) - (inv_mat(i,:)*inv_mat(j,i)) 
        !else
           !inv_mat(j,:) = arr(j,:)
        end if
     end do
...
!arcsin n=0,inf  sum -> (2n)! * x**(2n+1) / 4**n *(n!)**2 * (2n+1)
subroutine arcsin(x,ans)
  integer i
  real*8 term,ans,x
  ans = x
  term = 1.d0
  do i=1,50000
     term = term* (2.d0*i)/((4.d0)*i*i)
     ans = ans + term*(x**(2*i+1.d0))/(2.d0*i+1)
  end do
end subroutine arcsin

!arccos(x) = pi/2 - arcsin(x)

!arctan n=0,inf sum-> (-1)**n * x**(2n+1) / (2n+1)
subroutine arctan(x,ans)
  integer i
  real*8 term, ans, x
  ans = 0.d0
  term = 1.d0
  do i=1,50000
     term = ((-1.d0)**(i-1))/(2*(i-1)+1)
     ans = ans + term * x**(2*(i-1)+1)
  end do
