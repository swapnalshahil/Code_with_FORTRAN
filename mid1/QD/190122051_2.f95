...
PROGRAM main
    ...
    integer i,j,k
    real*8 dx,dt,xst,xend,x(nx),l, potential, v(nb,nb),phi(nx,nb),psi(nx)
    complex*16 iota, a(nb,nb), H(nb,nb),Cm(nb,1),Xt(1,1) 
    ...
    !        do k=1, grid_point
    !            x = xst + (k)*dx
    !            potential = (1.0/2.0)*x*x
    !          v(i,j) = v(i,j) + (sqrt(2.0/l) * sin(i*pi*((x-xst)/l)))*potential*(sqrt(2.0/l) * sin(j*pi*((x-xst)/l)))*dx
    !           xmat(i,j) = xmat(i,j) + (sqrt(2.0/l) * sin(i*pi * ((x - xst)/l))) * x * (sqrt(2.0/l) * sin(j*pi * ((x - xst)/l)))*dx
    !        end do
    !    end do
    !end do
    !Adding Kinetic at diogonal
    !do i=1,n
       ! v(i,i) = v(i,i) + (1.0/2.0)*(i**2 * pi**2)/(l**2)
   ! end do

    ! matrix a is -iota*dt*[V+K]
   ! a = -iota*dt*v
    !defining exp(A) as H = exp(A)
    !call complex_exponent_matrix(a,H, n)

    
   
    call calc_xi(xst,xend,dx,x,nx)
    call calc_phi(x,phi,nx,nb,l,xst)
    call calc_potential(v,x,phi,nx,nb)
    call calc_kinetic(v,l,nx,nb)
    call calc_psi(psi,x,nx)
    call calc_H(v,x,nx,nb,dt,H)
    
    !calculate Cm = psi * (sqrt(2.0/l) * sin(i*pi * ((x - xst)/l)))*dx
   
    do i=1,nb
        Cm(i,1) = 0
        do j=1,nx
            
            Cm(i,1) = Cm(i,1) +phi(j,i)*psi(j)*dx
        end do
    end do
    !do i=1,nb
    !   write(outunit,*) i, Cm(i,1)
    !end do
    !Calculate X(t)
    do i =1,nb
        Xt = matmul(transpose(conjg(Cm)), matmul(H,Cm))
        Cm = matmul(H,Cm)
        write(outunit,*) i*dt, real(Xt(1,1))
    end do
...
subroutine calc_xi(xst,xend,dx,x,nx)
  integer i,nx
  real*8 xst,xend,dx,x(nx)
  do i=1,nx
     x(i) = xst + (i)*dx
  end do
end subroutine calc_xi

subroutine calc_phi(x,phi,nx,nb,l,xst)
  integer i,nx,nb
  real*8 x(nx),phi(nx,nb),l,xst,pi
  pi = acos(-1.d0)
  do i=1,nb
     do j=1,nx
        phi(j,i) = 1.d0* sqrt(2.d0/l)*sin((i*pi*(x(j)-x(1))/l))
     end do
  end do
end subroutine calc_phi

subroutine calc_potential(v,x,phi,nx,nb)
  integer i,j,nx
  real*8 v(nb,nb),x(nx),phi(nx,nb)
  v = 0.d0
  do i=1,nb
    
        do j=1,nb
           do k=1,nx
              v(i,j) = v(i,j) + phi(k,i)*(((1.d0 + g)*x(k)**2)/2.d0) *phi(k,j)*dx
           end do
        end do
...
subroutine calc_kinetic(v,l,nx,nb)
  integer i,nx,nb
  real*8 v(nb,nb),l,pi
  pi = acos(-1.d0)
  
  do i=1,nb
     v(i,i) = v(i,i) + (i**2 * pi**2)/(2.d0*l**2)
  end do
...
subroutine calc_psi(psi,x,nx)
  ...
  real*8 psi(nx),x(nx)
  do i=1,nx
        psi(i) = (1.d0+x(i))*exp(-(x(i)**2)/2.d0)
  end do
end subroutine calc_psi

subroutine calc_H(v,x,nx,nb,dt,H)
  integer nx,nb
  real*8 v(nb,nb),x(nx),dt
  complex*16 H(nb,nb),a(nb,nb),iota
  iota = (0.d0,1.d0)
  a = -iota * dt * v
  call complex_exponent_matrix(a,H,nb)
