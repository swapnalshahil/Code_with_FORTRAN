...
PROGRAM main
    ...
    integer n,i,j,k ,grid_point
    real*8 dx,dt,xst,x,l, potential, v(100,100)
    complex*16 iota, a(100,100), H(100,100) ,Xt(1,1),  xmat(100,100) , Cm(100,1)
    ...
    xst = -10.0d0
    dx = 0.1d0
    l = (10.0 + (10.0))  !length of the box = xend - xstart
    dt = 0.1
    ...
    do i=1,n
        ...
        do j=1,grid_point
            x = xst + j*dx
            Cm(i,1) = Cm(i,1) +(sqrt(2.0/l) * sin(i*pi * ((x - xst)/l)))* exp(-x**2/2.0)*(1+x)*dx
        end do
    end do
    
    !Calculate X(t)
    do i =1,n
        Xt = matmul(transpose(conjg(matmul(H,Cm))), matmul(xmat,matmul(H,Cm)))
        Cm = matmul(H,Cm)
        write(outunit,*) i*dt, realpart(Xt)
    end do
